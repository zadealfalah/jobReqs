    ...
    try:
        ...
        if script_tag is not None:
            ...
            for i, job in enumerate(jobs_list):
                if (job.get('jobkey') is not None) & (job.get('jobkey') not in init.job_id_list):
                    init.job_id_list.append((job.get('jobkey'), keyword))
            # if len(jobs_list) < 10:
            #     break
...
def get_job_data(driver, job_id):
    ...
    if job_id[0] in init.job_data:
        ...
        if job_id[1] in init.job_data[job_id[0]]['terms']:
            pass
        else:
            init.job_data[job_id[0]]['terms'].append(job_id[1]) # add the search term used to the terms 
        
    else:  # new job ID
        init.job_data[job_id[0]] = {}  # create empty nested dict with job id as key
        init.job_data[job_id[0]]['terms'] = [] # create empty list to be appended to with search terms
        try:
            ...
            if script_tag is not None:
                ...
                if json_blob["salaryInfoModel"] is not None: 
                    init.job_data[job_id[0]]["salary_min"] = json_blob["salaryInfoModel"]["salaryMin"]
                    init.job_data[job_id[0]]["salary_max"] = json_blob["salaryInfoModel"]["salaryMax"]
                # If instead the salary is an estimate from indeed
                elif json_blob["salaryGuideModel"]["estimatedSalaryModel"] is not None:
                    init.job_data[job_id[0]]["salary_min"] = json_blob["salaryGuideModel"]["estimatedSalaryModel"]["min"]
                    init.job_data[job_id[0]]["salary_max"] = json_blob["salaryGuideModel"]["estimatedSalaryModel"]["max"]
                # If salary has none from company nor an estimate from indeed    
                else:
                    init.job_data[job_id[0]]["salary_min"] = None
                    init.job_data[job_id[0]]["salary_max"] = None    
                
                init.job_data[job_id[0]]['terms'].append(job_id[1]) # start the list of search terms with the term used here
                init.job_data[job_id[0]]['title'] = job.get('jobInfoHeaderModel').get('jobTitle') if job.get('jobInfoHeaderModel') is not None else ''
                init.job_data[job_id[0]]["company"] = job.get('jobInfoHeaderModel').get('companyName') if job.get('jobInfoHeaderModel') is not None else ''
                ...
                init.job_data[job_id[0]]['desc'] = no_html_desc
