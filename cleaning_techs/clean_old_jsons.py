###
# This is the lambda_function.py in this folder, but for older json files which were still erroneously replacing the gpt techs with cleaned techs
###
import json
import re
import boto3
...
def lambda_handler(event, context):
    ...
    for line in lines:
        ...
        ### Get the cleaned_techs key.
        ### Old files had the gpt responses replaced with what would have been there
        ### Extract the list, add it to the proper key
        data['cleaned_techs'] = data['choices'][0]['message']['content']
    ...
    save_key = f"data/{stripped_key}"
    
    try:
        s3.put_object(Bucket=save_bucket_name, Key=save_key, Body=modified_content)
